...
describe('StringExtra', function() {
  ...
  it('should return a StringExtra object', function() {
    const actual = S(test);
    assert.stringExtra(actual);
  });
  ...
  it('should return a string', function() {
    const actual = S(test);
    assert.stringExtra(actual).strictEqual(actual.s, test);
  });

  it('should typecast to a string', function() {
    const actual = S(test);
    assert.stringExtra(actual).strictEqual(`${actual}`, test);
  });
...
describe('StringExtra:chalk', function() {
  ...
  for (let i = 0, l = chalkProperties.length; i < l; i++) {
    ...
    it(`should chain the "${prop}" chalk property`, function() {
      ...
      assert.stringExtra(actual).strictEqual(actual.s, expected);
    });
  }
